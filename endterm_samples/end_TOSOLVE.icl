...
remVowel :: String -> String
remVowel a = {a\\a<-ls}
where ls = filter (\x = not(isMember x vowelList)) [aa \\ aa <-: a] 
//Start = remVowel "Hello"

removeVowels :: {{String}} -> {{String}}
removeVowels arr =  { {aa\\aa<-a} \\ a<-succesls}
where
	ls = [ [aa\\aa<-:a] \\ a <-: arr] // [["hello","world"],["good","morning"]]
	succesls = [ map remVowel x \\ x <- ls] 
...
FindLinesT stop1 stop2 transport = [x.number \\ x <- Linelist | isMember stop1 x.stops && isMember stop2 x.stops && transport == x.transport]
...
intersection [] [] = []
intersection seta setb = [ a\\a<-seta | isMember a setb]
...
ageOn :: {Person} -> {Int}
ageOn arrayp = {newAge a\\a<-:arrayp}
...
sumTree :: (TreeT Int) -> Int
sumTree LeafT = 0
sumTree (NodeT x l r) = x + sumTree l + sumTree r
//Start = sumTree tree2

sizeTree :: (TreeT Int) -> Int
sizeTree LeafT = 0
sizeTree (NodeT x l r) = 1 + sizeTree l + sizeTree r
//Start = sizeTree tree2

AvgNode :: (TreeT Int) -> Int
AvgNode LeafT = 0
AvgNode (NodeT x l r) = (sumTree tree) / (sizeTree tree)
where tree = (NodeT x l r)
//Start = AvgNode tree2

goodNodes :: (TreeT Int) -> Int
goodNodes LeafT = 0
goodNodes (NodeT x l r)
| AvgNode (NodeT x l r) == x = 1 + goodNodes l + goodNodes r
...
count Leaf = 0
count (Node a l m r)
| l <> Leaf && m <> Leaf && r <> Leaf = 1 + count l + count m + count r
...
biggestNode Leaf2 = 0
biggestNode (Node2 a l r)
| l == Leaf2 && r <> Leaf2 = maxList([a] ++ [biggestNode l] ++ [biggestNode r])
= maxList([biggestNode l] ++ [biggestNode r])
...
checkMaybe (a,b) cond1 cond2
| cond1 a && cond2 b = (Just (a+b,a*b))
| cond1 a && not(cond2 b) = (Just (a+b, 0))
| not(cond1 a) && cond2 b = (Just (a*b, 1))
= Nothing
