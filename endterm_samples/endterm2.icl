...
shiftAux [] = []
shiftAux [x:xs]
| x>=65 && x<=90 = [toChar x] ++ shiftAux xs
| x > 90 = [toChar (x-26)] ++ shiftAux xs

shiftCipher:: String Int -> String
shiftCipher str num = arr
where 
	ls = [(toInt s + num)\\s<-:str]
	alma = shiftAux ls
	arr = {a\\a<-alma}
...
AVL_prop_check (Node i l r)
| abs(height l - height r) <=1 = True && AVL_prop_check l && AVL_prop_check r
= False
...
TreetoLs Leaf = []
TreetoLs (Node i l r) = TreetoLs l ++ [i] ++ TreetoLs r
//Start = TreetoLs BsTree1

transformAux :: (Tree Int) [Int] -> (Tree Int)
transformAux Leaf _ = Leaf
transformAux (Node x le ri) list = Node (sum (filter(\i = i >= x) list)) (transformAux le list) (transformAux ri list)

transform :: (Tree Int) -> (Tree Int)
transform Leaf = Leaf
transform tree = transformAux tree (TreetoLs tree)
...
c) Multiply the smallest unique tuple with the array elements.
...
sumTuple (a,b,c) = abs(a + b + c)

alterArray :: {(Int, Int, Int)} -> {(Int, Int, Int)}
alterArray arr = {(fst3 a * fst3 mintpl, snd3 a * snd3 mintpl, thd3 a * thd3 mintpl)\\a<-:arr}
where 
	ls = [a\\a<-:arr | isUnique a]
	min = minList [sumTuple aa\\aa<-ls]
	mintpl = hd[a\\a<-ls | sumTuple a == min]
...
commonStr str1 str2 = {a\\a<-lsnew}

where 
	ls1 = [s\\s<-:str1]
	indexOf = hd[b\\a<-ls1 & b<-[0,1..] | a == ' '] + 1
	lsnew = drop indexOf ls1


//Start = commonStr "Jamal Mammad" "Maria Mammad"

getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage arr = commonStr (fst person).fullName (snd person).fullName
where 
	year = minList[(fst a).yearOfMarriage\\a<-:arr]
	person = hd[a\\a<-:arr | (fst a).yearOfMarriage == year && (snd a).yearOfMarriage == year]
