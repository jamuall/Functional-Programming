...
where
	toInt x = name + x.age + x.height
	where name = length[x\\x <-:x.firstName] 
...
isPrime :: Int -> Bool
isPrime 1 = False
isPrime x = length[a\\a<-[2,3..x-1]| x rem a == 0] == 0
//Start = isPrime 5

countryCheck :: Country -> Bool
countryCheck country
| n > 0 = isPrime n
=False
where n = length [c\\c<-:country.capital | c == 'i']
//Start = countryCheck Brazil  // True since Brasilia has 2 i, and 2 is a Prime.
//Start = countryCheck Spain

continentsPrimeI :: [Continent] -> [String]
continentsPrimeI ls  = [continent.contName \\ continent <- ls  | or [countryCheck country \\country <-:continent.countries] ]
...
shouldWePlay ls player = length[pl\\pl <- ls | pl.skillLevel >= player.skillLevel] >= 3
...
(!+!) ls num
| num < length ls && num >= 0 = Just (ls !! num)
= Nothing
...
occurence n ls = length[x\\x<-ls | x == n] 

f7 :: {Int} -> Bool
f7 arr = and[occurence x ls >= 2\\x<-ls]
where ls = [x\\x<-:arr]
...
filterColoredTree Leaf = []
filterColoredTree (Node a NodeColor ls) color type f1 f2
