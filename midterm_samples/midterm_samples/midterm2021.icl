...
tplAux :: [(Int,Int)] -> Int
tplAux [] = 0
tplAux ls = length [a \\  a<-ls | fst a > snd a]
//Start = tplAux[(1,2), (2,2), (3,2)]
tuples_with_property :: [[(Int,Int)]] -> [Int]
tuples_with_property ls = [tplAux a \\ a<- ls]
...
decreasing_list x = aa ++ reverse aa
where aa = [dclAux a ++ reverse (dclAux a) \\ a<-[x,x-1..1]]
...
matchIndex ls f = allTrue aa
where aa = [f(length a) == f n\\  a <- ls & n<-[1,2..(length ls)] ]
...
prime :: Int -> Bool
prime n = length[a \\ a<-[2,3..n-1] | n rem a == 0 ]==0
//Start = prime 2

digitsToList :: Int -> [Int]
digitsToList a
| a < 10 = [a]
= digitsToList (a /10)  ++ [a rem 10]

isPali :: Int -> Bool 
isPali x = digitsToList x == reverse(digitsToList x)
//Start = isPali 252

extractAux :: [Int] -> (Int, Int)
extractAux ls = (a,b)
where
	a = sum(filter prime ls)
	b = sum(filter isPali ls)
//Start = extractAux [10,121,20,23]
extract :: [[Int]] -> [(Int, Int)]
extract [] = []
extract ls = [extractAux aa \\ aa <- ls]
//Start = extract [] //[]
...
primeFactors 0 = []
primeFactors x = filter prime factors
where factors = [a \\ a<- [2,3..x] | x rem a == 0 ]
